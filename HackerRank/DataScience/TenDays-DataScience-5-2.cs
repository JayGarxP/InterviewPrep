...
class Solution {
    ...
    double accumChance = 0; 
    // Running just once means on exactly the first try, 
    // need to sum up the chances of first five tries being success for this problem.
    // "Probability that defect is found within the first five inspections"
for (int i = 0; i < n; i++)
{
    accumChance += GeoMetricDist(n - i, p);
}

        //Console.WriteLine(GeoMetricDist(n,p));
        Console.WriteLine(accumChance);
    }
