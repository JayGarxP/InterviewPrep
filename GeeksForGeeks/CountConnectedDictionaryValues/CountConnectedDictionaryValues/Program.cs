...
namespace CountConnectedDictionaryValues
{
    ...
    class Solution
    {
        ...
        { "F", "F" }
            ...
            foreach (Employee staff in Company)
            {
                ...
                foreach (KeyValuePair<string, string> bossID in dict)
                {
                    if (staff.ID == bossID.Value)
                    {
                        ...
                        if (bossID.Key != bossID.Value)
                        {
                            staff.directPeonCount++;
                            staff.totalPeonCount++;
                            staff.reportingIDs.Add(bossID.Key);
                        }
                    }
                }

                //https://stackoverflow.com/questions/10436813/check-if-value-already-exists
            ...
            foreach (Employee staff in Company) {
                if (staff.directPeonCount > 0) {
                    foreach (string peon in staff.reportingIDs) {
                        //staff.totalPeonCount += reporting staff peon count(s)

                        //LINQ
                          var reportingIN  = Company.FirstOrDefault(o => o.ID == peon);
                        if (peon != null && reportingIN.ID != staff.ID)
                        { //don't double count BigBoss 
                            staff.totalPeonCount += reportingIN.directPeonCount;
                        }
                    }
                }
            ...
            foreach (Employee staff in Company) {
                Console.WriteLine("Ya boy "+staff.ID+" has "+staff.totalPeonCount.ToString()+" henchmen beneath them.");
